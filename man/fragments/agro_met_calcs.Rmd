...
We can add columns using weather calculations to the `crop` tibble
using the `tidyverse` functions `map_dbl` if for one varying date and
`map_dbl2` for varying start and end dates. For more than two varying
parameters we can use `pmap`. These functions are from the `purrr` library.
